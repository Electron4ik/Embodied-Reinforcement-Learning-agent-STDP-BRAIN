...
class Brain:
    ...
    def __init__(self, n_inputs: int, n_outputs: int, seed: int = 0):
        ...
        self.trace  = np.zeros(self.NT)        # spike trace (визуализация)
    ...
    def _tick(self):
        NI, OUT_S = self.NI, self.OUT_START
        ...
        self.out_counts  += fired[OUT_S:].astype(float)
        self.in_accum    += fired[:NI].astype(float)

        # Spike trace (для визуализации)
        self.trace       *= 0.92
        self.trace[fired] += 1.0

        # Pre-synaptic trace (для STDP)
        self.pre_trace          *= self._k_pre
        self.pre_trace[fired]   += 1.0

        # Reward-modulated STDP на internal связях
        # Только когда есть активный модулятор — не тратим время зря
        mod = self.dopamine - self.pain
        if abs(mod) > 0.05 and fired.any():
            # dW[post, pre] = mod * spikes[post] * pre_trace[pre]
            # Это outer product: кто сейчас стрельнул (post) × чей трейс (pre)
            f = fired.astype(np.float32)
            dW = LR_STDP * mod * np.outer(f, self.pre_trace)

            # Применяем только к internal-internal связям
            # Input→Output обновляет отдельный RL механизм — не трогаем
            NI, NIN = self.NI, self.NIN
            int_slice = slice(NI, NI + NIN)
            dW_int = dW[int_slice, int_slice]
            self.W[int_slice, int_slice] += dW_int * self.conn[int_slice, int_slice]

            # Inhibitory нейроны остаются отрицательными
            inh_cols = slice(self.INH_START, self.INH_END)
            self.W[int_slice, inh_cols] = -np.abs(self.W[int_slice, inh_cols])

            np.clip(self.W[int_slice, int_slice], -W_MAX, W_MAX,
                    out=self.W[int_slice, int_slice])
    ...
    def step(self, inputs: list[float]) -> tuple[int, float]:
        ...
        # Rate coding: число → вероятность Poisson
        arr = np.array(inputs, dtype=np.float32)
        arr = np.clip((arr + 1.0) / 2.0, 0.0, 1.0)   # нормировка в [0, 1]
        for i, val in enumerate(arr):
            s = i * NEURONS_PER_IN
            self.input_rates[s:s+NEURONS_PER_IN] = INPUT_RATE_LO + val * (INPUT_RATE_HI - INPUT_RATE_LO)
